...
function tiktokScheduleDate(date: Date) {
...
export default function Home() {
  ...
  const startPublication = useCallback(async () => {
    ...
    setShowConsole(true);
    try {
      // TikTok validates this timestamp against the user's launch time. Keep
      // the calculation in the browser, like Rocket, instead of relying on a
      // server runtime clock that may be in another region.
      const scheduleStart = new Date(Date.now() + launchDelay * 60_000);
      const scheduleEnd = new Date(scheduleStart.getTime() + 365 * 24 * 60 * 60_000);
      const response = await fetch("/api/tiktok/launch", {
        ...
        body: JSON.stringify({
          ...
          schedule_start_time: tiktokScheduleDate(scheduleStart),
          schedule_end_time: tiktokScheduleDate(scheduleEnd),
          country,
          language,
          ages,
      ...
      <div className="flex flex-col gap-2 xl:flex-row">
        <label className="relative flex-1">
          ...
          <input
            ...
            className="rocket-input rocket-input--with-icon"
          />
        </label>
...
function CampaignActionModal({
  ...
  return createPortal(
    <div className="campaign-action-backdrop" role="presentation">
      <section
        role="dialog"
        aria-modal="true"
